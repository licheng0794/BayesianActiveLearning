...
[x1 x2] = ndgrid(linspace(-5, 5, num_points), linspace(-5, 5, num_points));
x_star = [x1(:) x2(:)];
...
results_unc = learn_gp_hyperparameters_unc(problem, model); % the results for active learning

% random sampling

HnlZ = cell(1, problem.num_evaluations);
map_hyperparameters_random = cell(1, problem.num_evaluations);


ind = randperm(num_points*num_points, problem.num_evaluations);

for i = 1:problem.num_evaluations
    x = x_star(ind(1:i), :);
    y = y_star(ind(1:i));
    
    map_hyperparameters_random{i} = minimize_minFunc(model, x, y);
    
    [~, dnlZ, ~, ~, ~, HnlZ{i}] = ...
        gp(map_hyperparameters_random{i}, model.inference_method, ...
        model.mean_function, model.covariance_function, model.likelihood, ...
        x, y);
end

% compare the uncertainty sampling
