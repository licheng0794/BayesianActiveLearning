...
%       minFunc(f, unwrap(initial_hyperparameters), options.minFunc_options);
  save inithyper.mat initial_hyperparameters
  [best_hyperparameter_values, best_nlZ] = ...
      minFunc(f, [initial_hyperparameters.cov; initial_hyperparameters.lik; initial_hyperparameters.mean], options.minFunc_options);

  for i = 1:options.num_restarts
%     hyperparameters = model.prior();
    hyperparameters = best_hyperparameter_values+2*rand(length(best_hyperparameter_values),1)-1;
    [hyperparameter_values, nlZ] = ...
        minFunc(f, unwrap2vec(hyperparameters), options.minFunc_options);
